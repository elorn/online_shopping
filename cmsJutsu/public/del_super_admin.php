...
$query = "DELETE FROM super_admin WHERE id = {$admin["id"]}
LIMIT 1";
$result_del_admin = mysqli_query($conn,$query);

if($result_del_admin && mysqli_affected_rows($conn) === 1){
    $_SESSION["msg"] = "The super admin {$admin["username"]} was Deleted successfuly";
    redirect("manage_admins.php");
}else{
  $_SESSION["msg"] = "The super admin {$admin["username"]} Delete operation was Failed";
  redirect("manage_admins.php");
}
